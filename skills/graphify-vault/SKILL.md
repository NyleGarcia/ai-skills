...
- **Wiki export**: `graphify export wiki` — writes `graphify-out/wiki/index.md` plus one curated article per community, agent-crawlable. Root `CLAUDE.md` already tells any session to prefer this over `GRAPH_REPORT.md`/raw grep for broad navigation when it exists — generate it (cheap, deterministic, no LLM cost) whenever you export so that directive has something to find.
- **Query instead of grep**: `graphify query "<question>"`, `graphify path "<A>" "<B>"`, `graphify explain "<concept>"`, `graphify god-nodes` — all read `graphify-out/graph.json` directly, no need to export first. **Run the vocab-expansion step first** (see `references/query.md` in the installer-owned `/graphify` skill): the matcher is literal case-folded substring, no stemming or synonyms, so a wording mismatch between your question and the graph's own node labels silently returns 0 hits — that reads as "not found," not as "doesn't exist."
...
`graphify hook install` adds a post-commit git hook that re-extracts changed code files and rebuilds `graph.json`/`GRAPH_REPORT.md` after every commit (doc/image changes still need a manual `--update`). Worth installing once per repo if `graphify-docs-gaps`/`graphify-docs-upgrade` will be run regularly — otherwise those skills are trusting a graph that's only as fresh as the last manual `extract`. `graphify hook status` / `graphify hook uninstall` to check or remove.
