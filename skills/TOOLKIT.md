...
- **`docker-optimize`**: Optimizes Docker images for size, build speed, and security via multi-stage builds, layer-cache ordering, minimal base images, BuildKit cache mounts, and .dockerignore. Use when an image is too large or slow to build, or when writing or reviewing a Dockerfile.
