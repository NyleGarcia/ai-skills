...
Turn technical docs and PDFs into navigable Obsidian-compatible graphs via the `graphify` skill: `graphify extract` -> `graphify export obsidian --dir docs/knowledge`.
...
- If a query surfaces something worth permanent record, hand-write it as a real note under `docs/` per `docs-vault` conventions (kebab-case, YAML `tags:`, `[[wikilinks]]`) and link it from `docs/Home.md` — don't promote raw graphify notes wholesale. See `graphify` skill for full directives.
